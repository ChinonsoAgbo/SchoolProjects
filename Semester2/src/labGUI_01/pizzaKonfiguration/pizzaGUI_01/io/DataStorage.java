package labGUI_01.pizzaKonfiguration.pizzaGUI_01.io;



import labGUI_01.pizzaKonfiguration.pizzaGUI_01.model.Order;
import labGUI_01.pizzaKonfiguration.pizzaGUI_01.model.Pizza;
...
public class DataStorage {
    ...
    public void writeOrderCSV(Order order)throws IOException {
        try (BufferedWriter outputStream = new BufferedWriter(new FileWriter("Semester2/src/labGUI_01/pizzaKonfiguration/pizzaGUI_01/io/testout.csv"))) {
            for (Pizza pizza:
                    order.getPizzas()  ) {
    ...
    public Order readOrderCSV() throws IOException {
        try (BufferedReader reader = new BufferedReader(new FileReader("Semester2/src/labGUI_01/pizzaKonfiguration/pizzaGUI_01/io/testout.csv"))) {
    ...
    public void writeOrderBin(Order order) throws IOException {
        try(DataOutputStream dataOutputStream = new DataOutputStream(new FileOutputStream("Semester2/src/labGUI_01/pizzaKonfiguration/pizzaGUI_01/io/bestellung.bin"))){
            for (Pizza pizza :
                    order.getPizzas()) {
                dataOutputStream.writeInt(pizza.getPrice()); // price in binary int umschreiben
    ...
    public Order readOrderBin() throws IOException {
        try(DataInputStream dataInputStream = new DataInputStream(new FileInputStream("Semester2/src/labGUI_01/pizzaKonfiguration/pizzaGUI_01/io/bestellung.bin"))){
            Order order = new Order();
            while (true) {
                try {
